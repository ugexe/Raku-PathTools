...
B<Returns> C<Str @paths>
    ...
    # file search; recursively find all files with extension `.pm6`
    my @p6-modules = @files.grep(*.IO.extension ~~ 'pm6')

    # directory search; find all local git repos
    my @git-repos  = @dirs.grep(*.IO.basename eq ".git");
