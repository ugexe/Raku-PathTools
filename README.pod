...
B<Args> C<$path, :$test, Bool :$f = True, Bool :$d = True, Bool :$r>
...
key -> value arguments passed in will be passed along to the internal 
C<mkdir> and C<dir> calls used. For instance, one may wish to pass the 
C<:$test> which internally defaults to C<none('.','..')>
...
B<Returns> C<Str @deleted-paths>
    ...
    # rm -rf tmp
    my @deleted-files = rm("tmp/{time}"), :r, :f, :d);
...
Creates a new folder, B<$path>, and will attempt to cleanup its contents at C<END { }>.

If B<$path> is not supplied, a path name will be generated automatically with L<tmpdir>
